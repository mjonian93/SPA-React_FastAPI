        ...
        if len(note.title) == 0 and len(note.note_body) == 0:
            raise HTTPException(
                status_code=400, detail={
                    ...
                    "msg": "Both 'title' and 'note_body' are empty. TGhese are optional attributes but at least one must be provided."
                })
...
def update_note(note_id: int, updated_note: NoteInput):
    if len(updated_note.title) == 0 and len(updated_note.note_body) == 0:
        raise HTTPException(status_code=400, detail={
            "status": "Error 400 - Bad Request",
            "msg": "The note's 'title' and 'note-body' can't be both empty"
        })
    db = DBSession()
    try:
